...
def extract_jobs_from_browser():
    ...
    print(f"[INFO] 抓到 {len(job_cards)} 筆職缺卡片")
    ...
    for card in job_cards:
        ...
        for line in lines:
            if not job_title and any(kw in line for kw in ["工程師", "行銷", "設計", "主管", "助理"]):
                job_title = line.strip()
            if not company and ("有限公司" in line or "股份有限公司" in line):
                company = line.strip()
...
def highlight_duplicates(jobs):
    ...
    for job in jobs:
        ...
        key_counter[key] += 1
        job_dict[key].append(job["card"])

    duplicates = [key for key, count in key_counter.items() if count >= 2]

    for company, title in duplicates:
        for card in job_dict[(company, title)]:
            driver.execute_script("""
                arguments[0].style.border = '3px dashed red';
                arguments[0].style.backgroundColor = 'rgba(255,0,0,0.1)';
            """, card)
    print(f"[INFO] 高亮完成，共標記 {len(duplicates)} 組重複職缺")
...
def test_iframe_access():
    ...
    print(f"[DEBUG] 在 iframe 中找到 {len(cards)} 筆職缺")

    # 切回主頁面（避免後續錯誤）
    driver.switch_to.default_content()

def execute_custom_script(js_filename: str, options: dict):
    if not switch_to_104_tab():
        return

    js_path = os.path.join(os.path.dirname(__file__), js_filename)
    if not os.path.exists(js_path):
        messagebox.showerror("錯誤", f"找不到 JS 檔案: {js_path}")
        return

    with open(js_path, "r", encoding="utf-8") as f:
        custom_js_func = f.read()

    json_args = json.dumps(options)
    ...
    return (async function() {{
        try {{
            ...
            if (!frameDoc) throw new Error("無法取得 iframe 內容");
            ...
            const result = highlightDuplicates({json_args}, frameDoc);
            return {{ success: true, count: result }};
        }} catch (err) {{
            console.error("執行 highlightDuplicates 錯誤：", err);
    ...
    def callback(result):
        if result and result.get("success"):
            count = result.get("count", 0)
            root.after(0, lambda: messagebox.showinfo("完成", f"成功高亮 {count} 筆重複職缺"))
        else:
            error_msg = result.get("error", "無法取得回應")
            print(f"[錯誤除錯] JS 錯誤：{error_msg}")
            root.after(0, lambda: messagebox.showerror("錯誤", f"執行失敗：{error_msg}"))

    run_threaded(lambda: callback(driver.execute_script(wrapper_js)))
